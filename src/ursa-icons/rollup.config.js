...
const iconPaths = glob.sync(path.join(iconBasePath, "*.svg"));
...
iconPaths.forEach((filename) => {
  ...
  iconTypes.push(
    `export declare const ${exportName}: React.FC<React.SVGProps<SVGSVGElement>>;`
  );
});
...
function svgBuild(options = {}) {
  ...
  return {
    ...
    async transform(source, id) {
      ...
      const jsCode = await transform(optimizedSvg, {}, svgrState);
